...
echo "[$params_runid]  Starting HG19-pipeline..."
...
################################################################################ 
############################################################################ map 
echo "[$params_runid]  - Mapping to hg19"
mkdir aln
for i in demux_parsed/*.gz; do echo "/hpc/hub_kind/rvanderweide/bin/miniconda3/bin/bowtie2 --quiet --very-sensitive -N 1 -p 1 -x /home/hub_kind/rvanderweide/data/MAb-ID/hg19/hg19 -U $i | samtools view -ubh -F 4  - | samtools sort -l 9 -o aln/$(basename $i | sed 's/fq.gz/bam/') - 2> aln/$(basename $i | sed 's/fq.gz/err/')"; done > $params_runid.filelists/$params_runid.aln.filelist
...
mkdir counts
## TTAA
for i in aln/*TTAA*.bam; do echo "python /home/hub_kind/rvanderweide/projects/AbID/AbID_pipeline/pipeline_parts/count_ligations_flatten_UMIs.py -vvv --offset -1 --outfile counts/$(basename $i | sed 's/bam/hdf5/') --min-mapq 10 --umi-length 6 --keep-n 1000 --min-editdistance 1 --pos-file /home/hub_kind/rvanderweide/data/MAb-ID/hg19/hg19_mabid.TTAA.posarray.hdf5 $i 2> counts/$(basename $i | sed 's/bam/log/')"; done > $params_runid.filelists/$params_runid.count.filelist
## GATC
for i in aln/*GATC*.bam; do echo "python /home/hub_kind/rvanderweide/projects/AbID/AbID_pipeline/pipeline_parts/count_ligations_flatten_UMIs.py -vvv --offset 0 --outfile counts/$(basename $i | sed 's/bam/hdf5/') --min-mapq 10 --umi-length 6 --keep-n 1000 --min-editdistance 1 --pos-file /home/hub_kind/rvanderweide/data/MAb-ID/hg19/hg19_mabid.GATC.posarray.hdf5 $i 2> counts/$(basename $i | sed 's/bam/log/')"; done >> $params_runid.filelists/$params_runid.count.filelist
